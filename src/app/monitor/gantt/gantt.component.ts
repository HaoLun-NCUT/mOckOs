...
export class GanttComponent {
    ...
    algorithmService = inject(AlgorithmService); // 用於獲取演算法服務
    ...
    private cpuService = inject(CpuService); // 用於獲取 Kernel 時間資料流
    ...
    ngOnInit(): void {
        ...
        this.kernelTimeSubscription = this.cpuService.getTimeAfterOneInstruction().subscribe((kernelTime) => {
            //從執行佇列中獲取 PCB 資料
            this.algorithmService.runningSpace.queue.forEach((pcb) => {
            ...
            scales: {
                x: {
                    ...
                    ticks: {
                        ...
                        font: function (context: any) {
                            ...
                            let fontSize = Math.round(chartWidth / 100); // 簡單的比例計算，可以自行調整係數
                            if (fontSize < 10) fontSize = 7; // 最小字體大小
                            if (fontSize > 20) fontSize = 15; // 最大字體大小
                            return {
                                size: fontSize,
                                weight: 'bold'
                            };
                ...
                y: {
                    ...
                    ticks: {
                        color: 'black',
                        font: function (context: any) {
                            const yActualCount = context.chart.data.labels ? context.chart.data.labels.length : 1; // 確保不為 0
                            const chartHeight = context.chart.height;
                            let fontSize = Math.round(chartHeight / yActualCount / 2); // 根據行程數量計算字體大小
                            return {
                                size: fontSize
                            };
                        },
